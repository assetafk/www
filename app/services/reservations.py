...
def cancel_reservation(db: Session, redis_client: Redis, *, reservation_id: UUID) -> Reservation:
    ...
    product = db.execute(select(Product).where(Product.id == reservation.product_id).with_for_update()).scalar_one()
...
def list_reservations(
    ...
    *,
    ...
    stmt = select(Reservation).where(where_clause).order_by(Reservation.created_at.desc()).limit(limit).offset(offset)
...
def sync_expired_reservations(
    ...
    *,
    ...
    for rid in candidate_ids:
        ...
        try:
            reservation = db.execute(
                select(Reservation).where(Reservation.id == rid).with_for_update(skip_locked=True)
            ).scalar_one_or_none()
